...
console.log('this is profile.uploadedFile',profile.uploadedFile);
    const displayImage = () => {
        ...
        if (isTrue === true && profile.uploadedFile != undefined) {
            return (
    ...
    const sendProfileInfo = () => {
        if (profile.username!='' && profile.firstName!='' && profile.lastName!=''&&profile.cohort!=''&& profile.currentWork!='' && profile.position!='' && profile.startDate!='' && profile.linkedIn!=''&& profile.github!='' && profile.portfolio!='' && profile.bio!='' && profile.uploadedFile!=''){
            dispatch({
                            ...
                            <div className="mr-60 inline">
                                ...
                                />
                                {
                                    knownStacks?.map((stack) => {
                                        return (
                                            <div id="alert-3" className="flex p-4 mt-4 mb-4 text-green-800 rounded-lg bg-green-50 dark:bg-gray-800 dark:text-green-400" role="alert">
